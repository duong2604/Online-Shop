...
const AddCompartmentAdmin: React.FC = () => {
  const [form] = Form.useForm();

  const [createCompartment] = useCreateCompartmentMutation();

  const navigate = useNavigate();

  const { refetch } = useCompartmentQuery();
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      await createCompartment(updatedValues); // Gửi dữ liệu lên máy chủ
  ...
  return (
    <>
      <h2 className="title-appoiment">Thêm số ngăn</h2>
      <Form
        form={form}
        name="validateOnly"
        layout="vertical"
...
export default AddCompartmentAdmin;
