...
import { useCreateHangMutation, useHangQuery } from "../../../services/hang";
...
const AddHangAdmin: React.FC = () => {
  const [form] = Form.useForm();

  const [createHang] = useCreateHangMutation();

  const navigate = useNavigate();

  const { refetch } = useHangQuery();
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      await createHang(updatedValues); // Gửi dữ liệu lên máy chủ
  ...
  return (
    <>
      <h2 className="title-appoiment">Thêm hàng</h2>
      <Form
        form={form}
        name="validateOnly"
        layout="vertical"
