...
const AddLockTypeAdmin: React.FC = () => {
  const [form] = Form.useForm();

  const [createLockType] = useCreateLockTypeMutation();

  const navigate = useNavigate();

  const { refetch } = useLockTypeQuery();
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      await createLockType(updatedValues); // Gửi dữ liệu lên máy chủ
  ...
  return (
    <>
      <h2 className="title-appoiment">Thêm kiểu khóa</h2>
      <Form
        form={form}
        name="validateOnly"
        layout="vertical"
...
export default AddLockTypeAdmin;
